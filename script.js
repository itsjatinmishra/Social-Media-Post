...
function handlePostCreation(event) {
  ...
  const author = 'YourName'; // You may get this dynamically or from another input
  const content = document.getElementById('postInput').value;
  const imageInput = document.getElementById('imageInput');
  const image = URL.createObjectURL(imageInput.files[0]); // Convert image file to URL
  ...
  // Render the posts with the new post included
  renderPosts();
